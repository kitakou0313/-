...
bool comp1(const pair<int, pair<int, int>> a, const pair<int, pair<int, int>> b)
{
...
int main()
{
    ...
    for (int i = 1; i <= V; i++)
    {
        int x, y;
        cin >> x >> y;
        cp.push_back(make_pair(i, make_pair(x, y)));
    }

    sort(cp.begin(), cp.end(), comp1);
    for (auto city = cp.begin(); city != cp.end() - 1; ++city)
    {
        edge ed = {city->first, (city + 1)->first, abs(city->second.first - (city + 1)->second.first)};
        edges.push_back(ed);
    }
    sort(cp.begin(), cp.end(), comp2);
    for (auto city = cp.begin(); city != cp.end() - 1; ++city)
    {
        edge ed = {city->first, (city + 1)->first, abs(city->second.second - (city + 1)->second.second)};
        edges.push_back(ed);
    }
