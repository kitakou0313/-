...
int main()
{
    ...
    cout << abs(snum - fnum) << endl;
}

/*Syakutori
cout << fixed << setprecision(10) << pow(2LL, height + 1) - 1 << endl;
        ...
        for (int j = 0; j < W; j++)
        {
            ...
            while (!q.empty())
            {
                ...
                for (int k = 0; k < 4; k++)
                {
                    ...
                    if (-1 < nx && nx < H && -1 < ny && ny < W && visi[nx][ny] == 0)
                    {
                        if (map[nx][ny])
                        {
                            ds[nx][ny] = ds[now.first][now.second] + 1;
                            visi[nx][ny] = 1;
